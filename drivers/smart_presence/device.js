...
const UNLOADED_SETTINGS = { error: "unloaded" };
...
function Device(driver, device_data) {
  ...
  this.loadSettings(
    function() {
      ...
      if (
        this.settings == null ||
        !this.settings.hasOwnProperty("away_delay")
      ) {
        console.error("Could not retrieve away delay from settings");
      } else {
        this.last_seen_at =
          new Date().getTime() - this.settings.away_delay * 1001;
      }
    }.bind(this)
  );
}
...
Device.prototype.shouldStressCheck = function() {
  ...
  var predicate =
    this.present &&
    this.getAwayDelayInMillis() - (now - this.last_seen_at) <
      this.getStressAtInMillis();
...
Device.prototype.getAwayDelayInMillis = function() {
  ...
  if (settings) {
    ...
    console.log(
      "Using a default of ",
      defaultAwayDelay,
      "millis for 'away_delay', as the settings are not loaded yet."
    );
...
Device.prototype.getStressAtInMillis = function() {
  ...
  if (settings) {
    ...
    console.log(
      "Using a default of ",
      defaultStress,
      "millis for 'start_stressing_at', as the settings are not loaded yet."
    );
...
Device.prototype.getHostTimeoutInMillis = function() {
  ...
  if (settings) {
    ...
    console.log(
      "Using a default of ",
      defaultHostTimeout,
      "millis for 'host_timeout', as the settings are not loaded yet."
    );
...
Device.prototype.getStressHostTimeoutInMillis = function() {
  ...
  if (settings) {
    ...
    console.log(
      "Using a default of ",
      defaultHostTimeout,
      "millis for 'stress_host_timeout', as the settings are not loaded yet."
    );
    return defaultHostTimeout;
  }
};
